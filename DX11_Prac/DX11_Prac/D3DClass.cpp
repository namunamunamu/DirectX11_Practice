...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	//출력에 대한 DXGI_FORMAT_R8G8B8A8_UNORM표시 형식에 맞는 모드 수를 가져옵니다
	...
	//비디오 카드 메모리 용량 단위를 메가바이트 단위로 저장합니다ㅏ.
	...
	//설정된 깊이버퍼 구조체를 사용하여 깊이 버퍼 텍스쳐를 형성합니다
	...
	//2D 렌더링을 위한 직교투영 행렬을 만듭니다. ortho
	m_orthoMatrix = XMMatrixOrthographicLH((float)screenWidth, (float)screenHeight, screenNear, screenDepth);

	return true;
}

void D3DClass::Shutdonw()
{
	//종료 전 위도우 모드로 설정하지 않으면 스왑 체인을 해제 할떄 예외가 발생합니다.
	if (m_swapChain)
	{
