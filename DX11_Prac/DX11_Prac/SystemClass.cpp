...
bool SystemClass::Initialize()
{
	...
	//m_input 객체 생성. 이클래스는 추후 사용자의 키보드 입력처리에 사용됩니다.
	...
	//m_Graphics 객체 생성. 그래픽 랜더링을 처리하기위한 객체
...
void SystemClass::Run()
{
	...
	//사용자로부터 종료 메시지를 받을떄까지 메시지루프를 돕니다
	while (true)
	{
		//윈도우 메시지를 처리합니다
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	if (FULL_SCREEN)
	{
		//풀스크린 모드로 지정했다면 모니터 화면 해상도를 데스크톱 해상도로 지정하고 색상을 32비트로 지정합니다.
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT umessage, WPARAM wparam, LPARAM lparam)
{
	switch (umessage)
	{
		...
		//그외 모든 메시지들은 시스템 클래스의 메시지 처리로 넘깁니다.
		default:
		{
