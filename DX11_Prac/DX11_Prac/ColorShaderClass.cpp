...
{
	//버텍스 및 픽셀 쉐이더를 초기화
	return InitializeShader(device, hwnd, L"../DX11_Prac/Color.vs", L"../DX11_Prac/Color.ps");
}
...
bool ColorShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX veiwMatrix, XMMATRIX projectionMatrix)
{
	//렌더링에 사용할 쉐이더 매개 변수를 설정
	if (!SetShaderParameters(deviceContext, worldMatrix, veiwMatrix, projectionMatrix))
	{
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	{
		//쉐이더 컴파일 실패시 에러메시지 출력
		if (errorMessage)
		{
			OutputShaderErrorMessage(errorMessage, hwnd, psFilename);
		}
		else//컴파일 오류가 아니라면 쉐이더 파일을 찾을 수 없는거임(아마도 ㅎ)
		{
...
{
	//행렬을 transpose하여 셰이ㅣ더에서 사용할 수 있게 합니다
	worldMatrix = XMMatrixTranspose(worldMatrix);
	viewMatrix = XMMatrixTranspose(viewMatrix);
	projectionMatrix = XMMatrixTranspose(projectionMatrix);

	//상수 버퍼의 내용을 쓸수잇도록 잠급니다
	...
	//점점 쉐이더의 상수 버퍼의 위치를 설정합니다
	unsigned bufferNumber = 0;

	//마지막으로 버텍스 쉐이더의 상수 버퍼를 바뀐 값으로 바꿉니다
